...
void inicializar();
int verificarVazia();
void removerMenores(int numero);
...
void inserir(int numero, int posicao) {
    ...
    // se foi alocada memoria, adiciona o novoNoh na lista
    if (novoNoh != NULL) {
...
void removerMenores(int numero) {
    ...
    if (!verificarVazia()) {
        // Criar um ponteiro auxiliar para percorrer os nós da lista
        No* aux = l.inicio;
        No* anterior = NULL;

        // Percorrer a lista
        while (aux != NULL) {
            // Verificar se o número do nó atual é menor que o número informado
            if (aux->dado < numero) {
                // Verificar se é o primeiro elemento da lista
                if (anterior == NULL) {
                    // Atualizar o início da lista para o próximo nó
                    l.inicio = aux->proximo;
                    free(aux);
                    aux = l.inicio;
                } else {
                    // Remover o nó atual, atualizando o ponteiro do nó anterior
